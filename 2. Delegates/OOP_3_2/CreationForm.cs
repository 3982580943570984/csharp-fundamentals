...
namespace Studying
{
    public partial class CreationForm : Form
    {
        ...
        {
            Regex regex = new("^[а-яА-Я]{4,}$");
            if (regex.IsMatch(city))
                return true;
            return false;
        }

        private static bool IsValidStreet(string street)
        {
            Regex regex = new("^[а-яА-Я]{4,}$");
        ...
        {
            Regex regex = new("^(?!0)\\d{1,5}$|100000");
        ...
        {
            Regex regex = new("^(?!0)\\d{1,3}$|^[1-9]0{1,2}[1-9]$|^1\\d{3}$|^20[0-2]\\d$|^202[0-3]$");
            if (regex.IsMatch(year))
                return true;
            return false;
        }

        private static bool IsValidNumberOfApartments(string numberOfApartments)
        {
            Regex regex = new("^(?!0)\\d{1,5}$|100000");
        ...
        private static bool IsValidNumberOfFloors(string numberOfFloors)
        {
            Regex regex = new("^(?!0)\\d{1,3}$|10000");
        ...
        private void PerformCreation_Click(object sender, System.EventArgs e)
        {
            if (IsValidTextBoxes())
            {
                ...
                var house = new House(City, Street, Type, Number, YearBuilt, NumberOfApartments, NumberOfFloors);
