...
namespace Studying
{
    public partial class MainForm : Form
    {
        ...
        private void PerformChange_Click(object sender, System.EventArgs e)
        {
            if (IsValidTextBoxes())
            {
                if (!House.IsCreated)
                {
                    MessageBox.Show("Произошло инстанцирование объекта класса House");
                    House.IsCreated = true;
                }
                House house = House.Instance;
                house.City = CityTextBox.Text;
                house.Street = StreetTextBox.Text;
                house.Type = TypeTextBox.Text;
                house.Number = ushort.Parse(NumberTextBox.Text);
                house.YearBuilt = ushort.Parse(YearBuiltTextBox.Text);
                house.NumberOfApartments = ushort.Parse(NumberOfApartmentsTextBox.Text);
                house.NumberOfFloors = ushort.Parse(NumberOfFloorsTextBox.Text);
            }
            else {
        ...
        private void Parameters_Click(object sender, System.EventArgs e)
        {
            ...
            MessageBox.Show(House.Instance.ToString());
        }
    }
}
